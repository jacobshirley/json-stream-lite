...
/** @internal Map of JSON-relevant byte values for efficient parsing. */
const BYTE_MAP = {
  QUOTATION: 34,
  n: 110,
  t: 116,
...
/**
 * Incremental JSON parser that yields key-value pairs as they are parsed.
 *
 * This parser processes JSON byte-by-byte and can handle streaming input.
 * It flattens nested objects and arrays using dot-notation for object keys
 * and bracket-notation for array indices.
 *
 * @example
 * ```typescript
 * const parser = new JsonKeyValueParser()
 * const json = '{"name": "Alice", "scores": [100, 95]}'
 *
 * // Feed the entire JSON at once
 * parser.feed(...new TextEncoder().encode(json))
 *
 * // Or feed byte by byte for streaming
 ...
 * @example
 * ```typescript
 * // Incremental parsing with streaming data
 * const parser = new JsonKeyValueParser()
 *
 * for (const byte of streamingData) {
 *     parser.feed(byte)
 *     for (const pair of parser.parseNext()) {
