  ...
  *members(): Generator<[JsonString, JsonValue]> {
  ...
  async *membersAsync(): AsyncGenerator<[JsonString, JsonValue]> {
    while (!this.buffer.atEof()) {
      ...
      let currentMember: IteratorResult<[JsonString, JsonValue]> | undefined =
        undefined;
  ...
  *items(): Generator<JsonValueType> {
  ...
  async *itemsAsync(): AsyncGenerator<JsonValueType> {
    while (!this.buffer.atEof()) {
      ...
      let currentItem: IteratorResult<JsonValueType> | undefined = undefined;
  ...
  *parse(): Generator<JsonKeyValuePair> {
    if (this.container instanceof JsonObject) {
      for (const [keyEntity, valueEntity] of this.container) {
        ...
        const finalKey = this.parentKey ? `${this.parentKey}.${key}` : key;

        if (value instanceof JsonArray || value instanceof JsonObject) {
          yield* new JsonKeyValueParser(this.buffer, value, finalKey).parse();
        } else {
      ...
      yield* new JsonKeyValueParser(this.buffer, value, this.parentKey).parse();
    }
  }
  ...
  async *parseAsync(): AsyncGenerator<JsonKeyValuePair> {
    while (!this.buffer.atEof()) {
      ...
      let currentPair: IteratorResult<JsonKeyValuePair> | undefined = undefined;
