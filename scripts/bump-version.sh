...
if [ -n "$1" ]; then
  ...
  # Make sure all packages json-stream-lite* depend on latest versions of other json-stream-lite* packages

  echo "Updating all json-stream-lite* dependencies to version $NEW_VERSION"

  find . -name 'package.json' -not -path '*/node_modules/*' | while read -r pkg_file; do
    # Only update if there are json-stream-lite* dependencies
    if grep -q '"json-stream-lite' "$pkg_file"; then
      tmp_file=$(mktemp)
      node -e "
        const fs = require('fs');
        ...
        ['dependencies', 'devDependencies'].forEach(depType => {
          if (pkg[depType]) {
            Object.keys(pkg[depType]).forEach(dep => {
              if (dep.startsWith('json-stream-lite')) {
